...
\begin{definition}
	If $E \subset B$ \smallmarginpar{$E$ is not necessarily a subset of $f(A)$}, $f^{-1}(E)$ denotes the set of all $x \in A$ such that $f(x) \in E$. We call $f^{-1}(E)$ the {\cem{inverse image}} of $E$ under $f$. If $y \in B$, $f^{-1}(y)$ is the set of all $x \in A$ such that $f(x) = y$. If, for each $y \in B$, $f^{-1}(y)$ consists of at most one element of $A$, then $f$ is said to be a 1-1 ({\cem{one-to-one}}) mapping of $A$ into $B$. This may also be expressed as follows. $f$ is a 1-1 mapping of $A$ into $B$ provided that $f(x_1) \ne f(x_2)$ whenever $x_1 \ne x_2, x_1 \in A, x_2 \in A$.
\end{definition}
...
\begin{definition}
	...
	\begin{enumerate}[(a)]
		...
		\item A point $p$ is a {\cem{limit point}} of the set $E$ if {\cem{every}} neighborhood of $p$ contains a point $q \ne p$ such that $q \in E$.
		\item If $p \in E$ and $p$ is not a limit point of $E$, then $p$ is called an {\cem{isolated point}} \smallmarginpar{An equivalent definition: There exsits a neighborhood of $p$ such that the only element {\underline{in $E$}} it contains is $p$ itself.} of $E$.

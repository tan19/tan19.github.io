...
Let $X$ and $Y$ be metric spaces; suppose $E \subset X$, $f$ maps $E$ into $Y$, and $p$ is a limit point of $E$. \smallmarginpar{The definition does not say anything about $f(p)$.} We write $f(x) \to q$ as $x \to p$, or $$\lim_{x \to p} f(x) = q$$ if there is a point $q \in Y$ with the following property: For every $\epsilon > 0$ there exists a $\delta > 0$ such that $$d_Y(f(x),q) < \epsilon$$ for all points $x \in E$ for which $$0 < d_X(x,p) < \delta.$$
\end{definition}
...
\subsection{Definitions}
\deff{
	Let $f$ be defined on $(a,b)$. Consider any point $x$ such that $a \le x < b$. We write $$f(x+) = q$$ if $f(t_n) \to q$ as $n \to \infty$, for all sequences $\{t_n\}$ in $(x,b)$ such that $t_n \to x$. To obtain the definition of $f(x-)$, for $a < x \le b$, we restrict ourselves to sequences $\{t_n\}$ in $(a,x)$. It is clear that any point $x$ of $(a,b)$, $\lim_{t \to x} f(t)$ exists if and only if $$f(x+) = f(x-) = \lim_{t \to x} f(t).$$ 
}
...
\subsection{Definitions}
\deff{
	Let $f$ be real on $(a,b)$. Then $f$ is said to be {\cem{monotonically increasing}} on $(a,b)$ if $a < x < y < b$ implies $f(x) \le f(y)$. If the last inequality is reversed, we obtain the definition of a {\cem{monotonically decreasing}} function. The class of monotonic functions consists of both the increasing and the deceasing functions.
}
...
\coro{
	Monotonic functions have no discontinuities of the second kind. \smallmarginpar{Compare with Corollary~\ref{coro: derivative functions do not have discontinuities of the first kind.}}
}\label{coro: monotone functions do not have discontinuities of the second kind.}

...
\ques{Tools: (1) A wrist watch and (2) a normal knife. Goal: Slice a pizza into 11 equal pieces}
\ans{
1)Position the watch hands to noon and put in the center of pizza
2)Cut in the direction of the overlapping watch hands
3)Advance the watch until next overlapping of its hands
Repeat steps 2 and 3 until all pieces are sliced
...
\chapter{Finance}
\section{Quant Finance Concept Questions}
\ques{How to handle the correlation between spot and instantaneous vol? If you have option portfolios, when you shock vol, what should you consider about?}
...
\chapter{Mathematics and Statistics}
\section{Calculus}
...
\ques{You are given two Gaussian variables: $X_1$ and $X_2$ with means $m_1$ and $m_2$ and variances $v_1$ and $v_2$}. Suppose you know the sum $X_1 + X_2$ is equal to $n$. What is the expected value of $X_2$?}

\ques{
	$X$ and $Y$ are i.i.d. standard normal. Conditioned on $X+Y=1$, what is $Cov(X,Y)$?
}
\ans{
	$\frac{1}{2}$. NOT $-1$!
}
\ques{
	A box has 50 checks: \$1, $\cdots$, \$50. You pay \$1 to draw one check, after that you either keep the check and leave, or put the check back and pay \$1 to play again. What is your best strategy?
}
\ques{
...
\ques{
	Given an array of $N$ numbers, find the smallest $K$ numbers of of it.
}

\ans{
	Use max heap (O(k + (n-k)logk)). Other choices: sorting (O(nlogn)), selection sort (O(nk)).

...
\section{Calculus}
\ques{Expand $e^x$ using Talyor expansion. When does $\sum_{i=1}^\infty \frac{x^n}{n!}$ converge? ({\bf{Goldman Sachs Strat}})}
...
\end{align}
But this is not a good model, since the interest rate $r_t$ must not be negative. How to fix it? Add a $\sqrt{r_t}$ term. This is called the CIR model:
\begin{align}
